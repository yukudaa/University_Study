...
class ChecksumMethod {
	public static void main(String args[]) {
		...
		System.out.println("영문이름과주소를 작성하시오1:");
	...
	static int generateChecksum(String s) {
		...
		for(i=0 ; i < s.length()-4 ; i=i+4) {
			...
			x = (int) (s.charAt(i+1));
			hex_value = hex_value + Integer.toHexString(x+1);
			x = (int) (s.charAt(i+2));
			hex_value = hex_value + Integer.toHexString(x+2);
			x = (int) (s.charAt(i+3));
			hex_value = hex_value + Integer.toHexString(x+3);
			...
			checksum += x;
			// Add 'x' into 'checksum' checksum에다가 x더함
		}
		if(s.length()%4 == 0) {
